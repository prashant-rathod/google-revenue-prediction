...
class DA_rnn(nn.Module):
    ...
    def __init__(self, input_size, X, y, T,
                 encoder_num_hidden,
                 decoder_num_hidden,
                 batch_size,
                 learning_rate,
        ...
        # Training set
        self.train_timesteps = int(self.X.shape[0] * train_split)
        self.input_size = input_size
    ...
    def test2(self, X_test):
        ...
        while i < len(y_pred):
            ...
            for j in range(len(batch_idx)):
                batch_range = range(batch_idx[j] + X.shape[0] - self.T, batch_idx[j] + X.shape[0] - 1)
                X[j, :, :] = self.X[batch_range, :]
                y_history[j, :] = self.y[batch_range]
